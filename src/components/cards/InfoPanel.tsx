...
export function InfoPanel({ title, meta, value, isFluid }: InfoPanelProps) {
  return (
    <View className="rounded-xl border border-border dark:border-dark-border bg-card dark:bg-dark-card p-4 shadow-sm" style={{ flex: isFluid ? 1 : undefined }}>
      <Text className="text-xs font-bold uppercase text-brand-600">{value}</Text>
      <Text className="mt-1 text-base font-bold text-foreground dark:text-dark-foreground">{title}</Text>
